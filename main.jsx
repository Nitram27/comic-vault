...
// Dropdown con posizione fixed per evitare clip dalle card
...
export default function App() {
  ...
  function openAddForm(){ setForm(emptyForm); setEditId(null); setSeriesMode(false); setSeriesSearch(""); setShowForm(true); }
        ...
        <div style={{ maxWidth:900, margin:"0 auto", padding:"14px 20px" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:14 }}>
            <div>
              <div style={{ display:"flex", alignItems:"center", gap:5 }}>
                ...
                <button onClick={()=>setShowSpesa(s=>!s)}
                  style={{ background:"none", border:"none", cursor:"pointer", fontSize:12, color:"#444", padding:0, lineHeight:1 }}>
                  {showSpesa?"👁":"🙈"}
                </button>
              </div>
            ...
            <button onClick={openAddForm}
              style={{ background:ACCENT, color:"#000", border:"none", padding:"9px 18px", fontWeight:800, fontSize:13, cursor:"pointer", textTransform:"uppercase", letterSpacing:1, borderRadius:8 }}>
              + Aggiungi
            </button>
          </div>
        ...
        <div style={{ display:"flex", gap:10, marginBottom:18, alignItems:"center" }}>
          ...
          <div style={{ display:"flex", border:"1px solid #2a2a2a", borderRadius:8, overflow:"hidden" }}>
            <button onClick={()=>setCompact(false)}
              style={{ padding:"8px 12px", background:!compact?"#2a2a2a":"transparent", border:"none", cursor:"pointer", fontSize:15, color:!compact?ACCENT:"#444" }}>▤</button>
            ...
            <div key={monthLabel} style={{ marginBottom:32 }}>
              ...
              {compact?(
                ...
                <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(260px,1fr))", gap:12 }}>
                  {items.map(comic=>(
                    <div key={comic.id} onClick={()=>setSelected(comic)}
                      style={{ background:"#111", border:`1px solid ${comic.status==="owned"?"#1f2f1f":"#2a2a1a"}`, cursor:"pointer", borderRadius:14, overflow:"visible" }}>
                      {/* Status bar sottile */}
